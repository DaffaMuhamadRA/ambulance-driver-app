...
export async function POST(request: Request) {
  try {
    ...
    // 1. Validate strictly required fields (berdasarkan skema NOT NULL di database)
    ...
    // Konversi aman untuk angka (allow null)
    const sanitizedKmAwal = (km_awal !== null && km_awal !== undefined && km_awal !== "") ? validateNumericInput(km_awal, 0) : null
    const sanitizedKmAkhir = (km_akhir !== null && km_akhir !== undefined && km_akhir !== "") ? validateNumericInput(km_akhir, 0) : null
    const sanitizedBiayaAntar = (biaya_antar !== null && biaya_antar !== undefined && biaya_antar !== "") ? validateNumericInput(biaya_antar, 0) : null
